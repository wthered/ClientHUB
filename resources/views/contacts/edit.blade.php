...
@push('styles')
	{{-- Reusing the show grid layout but adding edit-specific styles --}}
	...
	<div class="contact-edit-container">
		...
		<header class="module-header">
			<div class="header-title-group">
				<h1 class="module-title">Edit Contact</h1>
				<p class="module-subtitle">Updating information for <strong>{{ $contact->full_name }}</strong></p>
			</div>
			<div class="header-actions">
				<a href="{{ route('contacts.show', $contact->id) }}" class="btn-action">Cancel</a>
			</div>
		</header>

		<form action="{{ route('contacts.update', $contact->id) }}" method="POST" class="contact-edit-form">
			@csrf
			@method('PUT')
			...
			<div class="contact-edit-grid">
				...
				<section class="contact-form-card {{ $errors->hasAny(['first_name', 'last_name', 'job_title', 'account_id']) ? 'has-errors' : '' }}">
					...
					<div class="form-grid-2 mt-3">
						...
						{{-- Account Selection --}}
						<div class="form-group">
							<label for="account_id">Account (Company)</label>
							<select name="account_id" id="account_id"
							        class="form-select @error('account_id') is-invalid @enderror">
								@foreach($accounts as $account)
									<option value="{{ $account->id }}" {{ old('account_id', $contact->account_id) == $account->id ? 'selected' : '' }}>
										{{ $account->name }}
									</option>
								@endforeach
							</select>
				...
				<section class="contact-form-card {{ $errors->hasAny(['owner_id', 'is_primary', 'notes']) ? 'has-errors' : '' }}">
						...
						<div class="form-group">
							...
							<select name="owner_id" id="owner_id"
								...
								@foreach($users as $user)
									<option value="{{ $user->id }}" {{ old('owner_id', $contact->owner_id) == $user->id ? 'selected' : '' }}>
						...
						<div class="form-group" style="justify-content: end;">
							<div class="toggle-container">
								...
								<input type="checkbox" name="is_primary" id="is_primary" value="1"
								       class="toggle-input" {{ old('is_primary', $contact->is_primary) ? 'checked' : '' }}>
					...
					<div class="form-group mt-3">
						...
						<textarea name="notes" id="notes"
						          ...
						          rows="5">{{ old('notes', $contact->notes) }}</textarea>
						@error('notes')
						<span class="invalid-feedback">{{ $message }}</span>
						@enderror
					</div>
			...
			<div class="contact-form-actions">
				<button type="submit" class="btn-save">Save Contact Changes</button>
			</div>
		</form>
	</div>
@endsection